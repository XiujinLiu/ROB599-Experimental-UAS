...
static rc_filter_t D_pitch_i =         RC_FILTER_INITIALIZER;           //New
...
static void __rpy_init(void)
{
    ...
    rc_filter_duplicate(&D_pitch_i, settings.pitch_controller_i);             // New
...
static void __assign_setpoints_and_enable_loops()
{
    ...
    switch (user_input.flight_mode)
    {
        ...
        case MANUAL:
            ...
            // Handle Yaw Transition from Mocap Modegits to DIRECT_THROTTLE_4DOF
...
static void __run_attitude_controller()
{
    ...
    setpoint.pitch_dot  = rc_filter_march(&D_pitch,  setpoint.pitch  - state_estimate.pitch)
                       ...
                       + rc_filter_march(&D_pitch_i,  setpoint.pitch  - state_estimate.pitch);
