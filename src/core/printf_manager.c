...
static int __print_header()
{
    ...
    {
        printf("%s alt(m) |altdot| accel-z", __next_colour());
    }
...
static void* __printf_manager_func(__attribute__((unused)) void* ptr)
{
    ...
    while (rc_get_state() != EXITING)
    {
        ...
        if (settings.printf_altitude)
        {
            printf("%s%+5.2f |%+5.2f | %+5.2f", __next_colour(), state_estimate.alt_bmp,
                state_estimate.alt_bmp_vel, state_estimate.accel[2]);
        }
        if (settings.printf_battery)
        {
