...
s*+-tatic void __batt_march(void)
{
    double tmp = rc_adc_dc_jack();
...
static int __altitude_init(void)
{
    ...
    if (rc_filter_first_order_lowpass(&acc_lp, DT, 40 * DT)) return -1;
...
static void __altitude_march(void)
{
    ...
    state_estimate.alt_bmp_vel = alt_kf.x_est.d[1];  //u.d[0]; 	//alt_kf.x_est.d[1];
...
static void __feedback_select(void)
{
    switch (user_input.flight_mode)
    {
        ...
        default:
            ...
            state_estimate.Y = xbeeMsg.y;
            state_estimate.Z = xbeeMsg.z;		//state_estimate.alt_bmp;
            state_estimate.X_dot = xbee_x_dot;
            state_estimate.Y_dot = xbee_y_dot;
            state_estimate.Z_dot = xbee_z_dot;		//state_estimate.alt_bmp_vel;
            break;
    }
}
